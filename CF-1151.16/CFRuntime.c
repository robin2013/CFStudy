...
CFTypeRef _CFRuntimeCreateInstance(CFAllocatorRef allocator, CFTypeID typeID, CFIndex extraBytes, unsigned char *category) {
    if (__CFRuntimeClassTableSize <= typeID) HALT; // 类别ID越界则挂起
    CFAssert1(typeID != _kCFRuntimeNotATypeID, __kCFLogAssertion, "%s(): Uninitialized type id", __PRETTY_FUNCTION__);
    CFRuntimeClass *cls = __CFRuntimeClassTable[typeID];// 从类别列表中获取对应的Class
    if (NULL == cls) {
	return NULL;
    }
    if (cls->version & _kCFRuntimeRequiresAlignment) {
        // 如果version 包含_kCFRuntimeRequiresAlignment, 即自定义内存对齐, CFRuntime 要使用requiredAlignment字段(用户定义的对齐系数), 并忽略参数中的 allocator 值
        allocator = kCFAllocatorSystemDefault;
    }
    Boolean customRC = !!(cls->version & _kCFRuntimeCustomRefCount);
    // 如果version 包含_kCFRuntimeCustomRefCount, CFRuntime 要使用refcount字段
    if (customRC && !cls->refcount) {// refcount无内容
    ...
    if (cls->version & _kCFRuntimeRequiresAlignment) {// 需要自定义对齐
	...
	*(CFAllocatorRef *)((char *)memory) = (CFAllocatorRef)CFRetain(realAllocator);
        // 如果不是系统默认, 在分配空间时, 多分配了sizeof(CFAllocatorRef)
    ...
    *cfinfop = (uint32_t)((rc << 24) | (customRC ? 0x800000 : 0x0) | ((uint32_t)typeID << 8) | (usesSystemDefaultAllocator ? 0x80 : 0x00));
    memory->_cfisa = 0;
    if (NULL != cls->init) {// 如果有init方法
	(cls->init)(memory);// 传入分配好的内存地址,进行初始化
    }
    return memory;// 返回对象地址
}
